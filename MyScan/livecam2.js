	...
	var wrap = function () {
		...
		socket.connect(gst_tcp_port, gst_tcp_addr, function () {
			var io = SocketIO.listen(
				server.listen(broadcast_tcp_port, broadcast_tcp_addr));
			...
			dicer.on('error', (e) => { console.log(e); });
...
function LiveCam(config) {
	...
	let gst_cam_process = null;
	let server = null;
	var broadcast = function () {
		var gst_cam_wrap = new SocketCamWrapper(gst_tcp_addr, gst_tcp_port, broadcast_addr, broadcast_port);
